...
const _float fCombo2to3Time = 0.2f;
const _float fAttackMove = 0.4f;
CState_PlayerAttack2::CState_PlayerAttack2(class CFsm* pFsm)
    :CState(pFsm)
{
}

HRESULT CState_PlayerAttack2::Initialize(_uint iStateNum)
{
    m_iStateNum = iStateNum;
    return S_OK;
}

HRESULT CState_PlayerAttack2::Start_State()
{
    m_pPlayer = static_cast<CPlayer*>(m_pFsm->Get_Owner());
    m_pPlayer->Change_Animation(TEXT("Attack2"));
    m_pPlayer->Set_Player_State(CPlayer::ATTACK2);
...
void CState_PlayerAttack2::Update(_float fTimeDelta)
{
    ...
    else if (m_pGameInstance->Get_KeyState(KEY::LBUTTON) == KEY_STATE::TAP && m_fComboTimer >= fCombo2to3Time)
    {
        m_pPlayer->Change_State(CPlayer::ATTACK3);
        m_pPlayer->Get_Player_Coll()->Set_IsActive(false);
    }

}

void CState_PlayerAttack2::End_State()
{
}

void CState_PlayerAttack2::Check_Move()
{
...
void CState_PlayerAttack2::Check_Mouse()
{
...
CState_PlayerAttack2* CState_PlayerAttack2::Create(class CFsm* pFsm,_uint iStateNum)
{
    CState_PlayerAttack2* pInstance = new CState_PlayerAttack2(pFsm);
    if (FAILED(pInstance->Initialize(iStateNum)))
    {
        MSG_BOX(TEXT("Failed to Created : CState_PlayerAttack2"));
        Safe_Release(pInstance);
    }
    return pInstance;
}

void CState_PlayerAttack2::Free()
{
    __super::Free();
}
